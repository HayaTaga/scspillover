...
Rcpp::List sar_full_sampler_cpp(const arma::vec& Y0_pre,    // T0
                                ...
                                Rcpp::Nullable<Rcpp::NumericVector> alpha_ = R_NilValue, // ★追加
                                int iteration = 2000, int burn = 1000,
                                double step_rho = 0.01,
                                double c_beta_ridge = 0.0,
                                double c_lambda_ridge = 0.0,
  ...
  // ★ α の有無で分岐できるよう準備
  const bool useAlpha = alpha_.isNotNull();
  arma::vec alpha;
  if (useAlpha) alpha = Rcpp::as<arma::vec>(alpha_);

  // ρ の安定域（従来どおり W のスペクトル境界でガード）
  ...
  arma::cube Lambda_draws(N, p, M, arma::fill::zeros);
  arma::cube F_draws(p, T0, M, arma::fill::zeros);

  // 状態（既存）
  double rho = 0.0, s2 = 1.0;
  arma::vec  beta0 = (K>0 ? arma::zeros<arma::vec>(K) : arma::vec());
  arma::vec  sig2_b0 = (K>0 ? arma::ones<arma::vec>(K) : arma::vec());
  arma::vec  nu_sig_b0 = (K>0 ? arma::ones<arma::vec>(K) : arma::vec());
  ...
  // ★ 共通ヘルパ：M(r) と μ(t; r)
  auto M_of = [&](double r)->arma::mat {
    arma::mat M = I_N - r * A;
    if (useAlpha) M -= r * (a * alpha.t());   // ← ρ w αᵀ 付き
    return M;
  };
  auto mu_of = [&](double r, int t)->arma::vec {
    arma::vec mu(N, arma::fill::zeros);
    if (!useAlpha) mu += r * a * Y0[t];       // ← αが無いときのみ ρ w y0_t を入れる
    if (useX) mu += X_get_row(t) * (K>0 ? beta0 : arma::vec());
  ...
  auto ll = [&](double r)->double {
    ...
    double ss = 0.0;
    for (int t=0; t<T0; ++t) {
      arma::vec u = M * Yc.row(t).t() - mu_of(r, t);
      ss += arma::dot(u,u);
    }
    return T0 * ldet - 0.5 * (N*T0) * std::log(s2) - 0.5 * ss / s2;
  };

  int iters = M + burn, acc = 0;
  for (int it=0; it<iters; ++it) {

    // （因子・Eta・β の更新は既存と同じ。★中の「残差」組み立てだけ M_of/ mu_of に置換）
    ...
    if (useX) {
      ...
      for (int t=0; t<T0; ++t) {
        ...
        Ab += Xt.t() * Xt;
        arma::vec Btmp = M_of(rho) * Yc.row(t).t() - mu_of(rho, t);  // ★ここだけ置換
        Bb += Xt.t() * Btmp;
      }
      // …以下は従来どおり（HS の対角を Ab.diag() に足して mvnrnd）
    ...
    double ss = 0.0;
    for (int t=0; t<T0; ++t) {
      arma::vec u = M_of(rho) * Yc.row(t).t() - mu_of(rho, t);  // ★ここだけ置換
      ss += arma::dot(u,u);
    }
...
Rcpp::List sar_full_one_step_cpp(const arma::vec& Y0_pre,    // T0
                                 ...
                                 double a0 = 1.0, double b0 = 1.0,
                                 bool verbose=false,
                                 Rcpp::Nullable<Rcpp::NumericVector> alpha_ = R_NilValue
                                ) {
  Rcpp::RNGScope scope;
  ...
  if (useX) {
    ...
    for (int t=0; t<T0; ++t) {
      ...
      arma::vec Btmp = M_of(rho) * Yc.row(t).t() - mu_of(rho, t);
  ...
  for (int t=0; t<T0; ++t) {
    ...
    arma::vec u = M_of(rho) * Yc.row(t).t() - mu_of(rho, t);
    ss += arma::dot(u,u);
  }
  double shape = a0 + 0.5 * (T0 * N);
  double rate  = 1.0/std::max(nu_sigma2,1e-12) + 0.5 * ss;
