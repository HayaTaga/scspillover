...
ggsave(
  filename = "inst/figures/fig_ca_diag.png",
  plot = p,
  width = 12.0,
  height = 6.0,
  dpi = 300,
...
ggsave(
  "inst/figures/fig_ca_weight.pdf",
  height = 5.5,
  width = 11
)
```
...
```{r}
a <- read_csv("data/raw/IMF_trade.csv")
a %>%
  filter(COUNTRY == "Egypt, Arab Republic of") %>%
  filter(COUNTERPART_COUNTRY == "Sudan") %>%
  count(TIME_PERIOD)
a %>%
  filter(COUNTRY == "Sudan") %>%
  filter(COUNTERPART_COUNTRY == "Egypt, Arab Republic of") %>%
  count(TIME_PERIOD)
a %>% select(COUNTRY) %>% unique()
b <- read.csv(
  "data/raw/P_Data_Extract_From_World_Development_Indicators/24cc87ae-8d91-47e6-b829-e467008e1958_Data.csv",
  na = c("..", ""),
  stringsAsFactors = FALSE
)
b %>% select(Country.Name) %>% unique()
