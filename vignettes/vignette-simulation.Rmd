    ...
    mutate(
      ...
      data = map(
        ...
        ~ readr::read_csv(.x, show_col_types = FALSE) %>%
          select(-any_of(c("N", "T0", "T1")))  # CSV 側の重複列は落とす
      )
    )
  print(meta)
  res <- meta %>%
...
make_table1_panel_lines <- function(res, T0_val, T1_val, panel_label) {
  ...
  # 列フォーマット：l + 3列×|N| 個
  col_spec <- paste0("l", paste(rep("rrr", length(N_vals)), collapse = ""))

  # cmidrule の範囲 (例: 3-5, 6-8, 9-11)
...
make_table2_lines <- function(mc_res, method_proposed = "Proposed") {
  ...
  t0_vals  <- sort(unique(cov_df$T0))  # 想定: 20, 50
  ...
  for (r in rho_vals) {
    ...
    line_r <- sprintf(
      "% .1f & %s & %s & %s & & %s & %s & %s \\\\",
      r, vals20[1], vals20[2], vals20[3],
      vals50[1], vals50[2], vals50[3]
    )
...
write_simulation_tables <- function(
  mc_res,
  T1_val = 20,
  file = "inst/tables/simulation_tables.tex"
) {
  # パネル (a): T0 = 20
  ...
  # パネル (b): T0 = 50
  lines_panel_b <- make_table1_panel_lines(
    res         = mc_res,
    T0_val      = 50,
    T1_val      = T1_val,
    panel_label = "b"
...
write_simulation_tables(
  mc_res,
  T1_val = 20,  # 必要なら変更
  file   = "inst/tables/simulation_results.tex"
)
