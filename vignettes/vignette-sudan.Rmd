...
load("./data/sudan_secession_up.rda")

panel_df <- sudan_secession_up$panel
w_vec <- sudan_secession_up$w
W_mat <- sudan_secession_up$W
panel_df <- panel_df %>%
  mutate(
...
ggsave(
  filename = "inst/figures/fig_sudan_diag.pdf",
  plot = p,
  width = 6.8,
  height = 4.5,
  dpi = 300,
...
write.csv(diag_tab, "inst/tables/sudan_diag_table.csv", row.names = FALSE)
...
cat(latex_code, file = "inst/tables/sudan_diag_table.tex")
```
...
p_a <- plot_panel_outcomes(
  ...
  skip_year = 2011,
  save_path = "inst/figures/fig_sudan_panelA.pdf"
)
...
ggsave(
  filename = "inst/figures/fig_sudan_panelB.pdf",
  plot = p_b,
  width = 6.8,
  height = 4.5,
  dpi = 300,
...
ggsave(
  filename = "inst/figures/fig_sudan_panel_spillover.pdf",
  plot = p_spill,
  width = 6.8,
  height = 4.5,
  dpi = 300,
...
sens$theta_table %>%
  ...
  save_kable("inst/tables/sudan_sens_table.tex")

write.csv(
  sens$theta_table,
  "inst/tables/sudan_sensitivity.csv",
  row.names = FALSE
)
```
...
gg <- ggplot(sim_long, aes(x = simulated_value)) +
  ...
  labs(
    title = "Prior Predictive Analysis (Sudan)",
    x = "Statistic Value",
    y = "Frequency"
  )
print(gg)
...
ggsave(
  filename = "inst/figures/fig_sudan_prior_predictive.pdf",
  plot = gg,
  width = 6.8,
  height = 4.5,
  dpi = 300,
...
write.csv(
  ppc_table,
  "inst/tables/sudan_ppa_summary_table.csv",
  row.names = FALSE
)
...
save_kable(ppa_kable_object, "inst/tables/sudan_ppa_summary_table.tex")
